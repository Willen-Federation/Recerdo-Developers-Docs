...
│  ┌─────────────────────────────────────────────────────────┐│
│  │ HTTP Server | PostgreSQL | Redis | SQS | asynq Worker  ││
│  └─────────────────────────────────────────────────────────┘│
└─────────────────────────────────────────────────────────────┘
                              △
                              │
...
| **HighlightVideo** | アルバムから自動生成されたビデオ | id (UUID), album_id, video_media_id (Storage ref), duration_secs, status (PENDING/GENERATING/READY/FAILED), generated_at? |
...
- **ハイライト自動生成**: media_count >= 10 時、自動トリガー（asynq ジョブ）
...
| **HighlightVideoReady** | GenerateHighlightVideo 完了 | album_id, video_media_id, duration_secs |
...
// HighlightVideo: 自動生成ハイライトビデオ
type HighlightVideo struct {
    ID           uuid.UUID
    AlbumID      ulid.ULID
    VideoMediaID string                // Storage Service 内 video media ID
    DurationSecs int
    Status       HighlightVideoStatus  // PENDING, GENERATING, READY, FAILED
    GeneratedAt  *time.Time
    CreatedAt    time.Time
    UpdatedAt    time.Time
}

// MarkAsReady: 生成完了をマーク
func (hv *HighlightVideo) MarkAsReady(duration int) error {
    if hv.Status != HighlightVideoStatusGenerating {
        return ErrInvalidStatusTransition
    }
    hv.Status = HighlightVideoStatusReady
    hv.DurationSecs = duration
    now := time.Now()
    hv.GeneratedAt = &now
    hv.UpdatedAt = now
    return nil
...
// MarkAsFailed: 生成失敗をマーク
func (hv *HighlightVideo) MarkAsFailed() error {
    if hv.Status != HighlightVideoStatusGenerating {
        return ErrInvalidStatusTransition
    }
    hv.Status = HighlightVideoStatusFailed
    hv.UpdatedAt = time.Now()
    return nil
}
...
const (
    HighlightVideoStatusPending    HighlightVideoStatus = "PENDING"
    HighlightVideoStatusGenerating HighlightVideoStatus = "GENERATING"
    HighlightVideoStatusReady      HighlightVideoStatus = "READY"
    HighlightVideoStatusFailed     HighlightVideoStatus = "FAILED"
)
```
...
| **AddMediaToAlbum** | User (Contributor) | メディアをアルバムに追加（ハイライト自動生成判定） | album_id, media_id, uploader_id, caption? |
...
| **GenerateHighlightVideo** | System (asynq) | メディア ≥10 時、ハイライトビデオ自動生成 | album_id |
...
// GenerateHighlightVideoInput
type GenerateHighlightVideoInput struct {
    AlbumID string `validate:"required,ulid"`
}

// GenerateHighlightVideoOutput
type GenerateHighlightVideoOutput struct {
    HighlightVideoID string `json:"highlight_video_id"`
    Status           string `json:"status"`
    JobID            string `json:"job_id"`  // asynq job ID
}
...
type AlbumRepository interface {
    ...
    // FindOrCreateHighlightVideo: ハイライトビデオを取得、なければ作成
    FindOrCreateHighlightVideo(ctx context.Context, albumID ulid.ULID) (*domain.HighlightVideo, error)
    
    // UpdateHighlightVideoStatus: ハイライトビデオステータス更新
    UpdateHighlightVideoStatus(ctx context.Context, videoID uuid.UUID, status string) error
}
```
...
type JobQueue interface {
    // EnqueueHighlightGenerationJob: ハイライト生成ジョブをエンキュー
    EnqueueHighlightGenerationJob(ctx context.Context, albumID ulid.ULID) (jobID string, err error)
}
...
type EventPublisher interface {
    ...
    // PublishHighlightVideoReady: ハイライト完了イベント発行
    PublishHighlightVideoReady(ctx context.Context, event *domain.HighlightVideoReadyEvent) error
}
```
...
| **GenerateHighlightJobHandler** | - | (asynq) | ハイライト生成ジョブ実行 |
...
| アダプタ | 技術 | 説明 |
|---------|------|------|
| **PostgresAlbumRepository** | pgx/v5 | Album, AlbumMedia, AlbumComment, HighlightVideo テーブル操作 |
...
CREATE TABLE IF NOT EXISTS highlight_videos (
    id UUID PRIMARY KEY NOT NULL,
    album_id ULID NOT NULL UNIQUE REFERENCES albums(id) ON DELETE CASCADE,
    video_media_id VARCHAR(255),
    duration_secs INTEGER,
    status VARCHAR(50) NOT NULL CHECK (status IN ('PENDING', 'GENERATING', 'READY', 'FAILED')),
    generated_at TIMESTAMPTZ,
    created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP
);

CREATE INDEX idx_highlight_status ON highlight_videos (status, created_at DESC);
```

### 6.3 メッセージブローカー
- **AWS SQS**: ドメインイベント非同期処理
...
| **aws-sdk-go-v2** | SQS イベント発行 | v1.24+ |
...
var AlbumModule = fx.Module("album",
    fx.Provide(
        ...
        // リポジトリ実装
        repository.NewPostgresAlbumRepository,
...
│   ├── adapter/
│   │   └── postgres_album_repository_test.go
│   ├── integration/
│   │   └── end_to_end_test.go
│   └── fixtures/
│       └── sample_albums.json
...
// adapter/postgres_album_repository_test.go: 統合テスト
...
func TestPostgresAlbumRepository_Create(t *testing.T) {
    ...
    pool := postgres.Pool()
    repo := repository.NewPostgresAlbumRepository(pool)
...
| フェーズ | 期間 | 作業 | リスク |
|---------|------|------|--------|
| **フェーズ 1** | W1-2 | PostgreSQL テーブル作成、FK 設定 | スキーマ設計誤り |
