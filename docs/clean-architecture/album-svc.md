...
## 1. 概要
### 1.1 目的
AlbumApp Service は、Recerdo プラットフォーム内で、イベントに紐付いたアルバム（写真・ビデオ集約）、メディアアソシエーション（Storage Service への参照）、および **ユーザー選択によるハイライトビデオ連結**を管理します。ユーザ間でイベント時の思い出を共有、保存、キュレーションするための中核機能を提供します。ハイライトは ML による自動生成を行わず、ユーザーが明示的に選択した複数メディアを FFmpeg concat で連結して HLS として書き出すのみです（[基本的方針](../core/policy.md) 参照）。

### 1.2 ビジネスコンテキスト
- **主要ユースケース**: アルバム作成・閲覧、メディア追加・削除、コメント、ユーザー選択ハイライトビデオ連結（自動生成なし）
...
- **非同期処理**: QueuePort（Beta: Redis+BullMQ/asynq, Prod: OCI Queue）経由で、ユーザー指定素材を HLS セグメント結合
- **イベント発行**: QueuePort → Timeline Service, Notification Service
- **ハイライトビデオはユーザー指定のみ**: 自動生成・ML による推薦は行わない
...
```
┌─────────────────────────────────────────────────────────────┐
│    External Systems (Storage, Events, QueuePort, Redis)     │
└─────────────────────────────────────────────────────────────┘
                              △
                              │
┌─────────────────────────────────────────────────────────────┐
...
│  ┌─────────────────────────────────────────────────────────┐│
│  │ HTTP Server | MySQL(MariaDB互換) | Redis | QueuePort ││
│  └─────────────────────────────────────────────────────────┘│
└─────────────────────────────────────────────────────────────┘
                              △
                              │
...
- **メディア最適化処理**: メディアについて各デバイスに最適なファイルを配信できるようにファイルを処理、自動トリガー（QueuePort 経由のジョブ）
...
// CanAddMedia: メディア追加可能判定
func (a *Album) CanAddMedia() bool {
    return a.IsEditable() && a.MediaCount < 10000 // 上限設定
}

// ハイライトビデオはユーザー指定のみ、自動生成しない。
// ポリシーにより MediaCount に基づく自動トリガーや ML による推薦は行わない。
...
type StorageService interface {
    ...
    // CreateHighlightVideo: ハイライトビデオを作成（オブジェクトストレージの署名付き URL 返却）
    // ハイライトビデオはユーザー指定のみ、自動生成しない。
    // mediaIDs はユーザーが選択した HLS 素材。FFmpeg でセグメントを結合して再生成する。
...
type EventPublisher interface {
    // PublishAlbumCreated: AlbumCreated イベント発行 → QueuePort (Beta: Redis, Prod: OCI Queue)
    ...
    // PublishOptimizationFileReady: メディア変換完了イベント発行
    // (FFmpeg HLS 変換 / libheif HEIC→JPEG/WebP 変換完了時。AWS MediaConvert は使用しない)
    PublishOptimizationFileReady(ctx context.Context, event *domain.MediaTranscodeReadyEvent) error
}
```
...
| **MediaTranscodeJobHandler** | -           | (QueuePort worker: FFmpeg HLS / libheif HEIC)         | 最適化ファイル生成（AWS MediaConvert は使用しない） |
...
| アダプタ                      | 技術        | 説明                                                         |
| ----------------------------- | ----------- | ------------------------------------------------------------ |
| **MySQLAlbumRepository**      | go-sql-driver/mysql | Album, AlbumMedia, AlbumComment, HighlightVideo テーブル操作 |
...
| **RedisBullMQAdapter**           | asynq / BullMQ | Beta: Redis ベースのジョブキュー |
| **OCIQueueAdapter**              | oci-go-sdk     | Prod: OCI Queue ベースのジョブキュー |
| **QueueEventPublisherAdapter**   | QueuePort      | ドメインイベントを QueuePort 経由で発行 |
...
```sql
-- MySQL 8.0 / MariaDB 10.11 互換スキーマ
-- （PostgreSQL 固有型 TIMESTAMPTZ / JSONB / UUID / partial index は不使用）
...
CREATE TABLE IF NOT EXISTS albums (
    ...
    status        VARCHAR(20) NOT NULL,                   -- ENUM 相当（ACTIVE/ARCHIVED）
    media_count   INT         NOT NULL DEFAULT 0,
    created_at    DATETIME(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6),
    updated_at    DATETIME(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6),
    PRIMARY KEY (id),
    CONSTRAINT chk_album_status CHECK (status IN ('ACTIVE','ARCHIVED'))
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;

CREATE INDEX idx_album_event_id ON albums (event_id);
CREATE INDEX idx_album_org_id_created ON albums (org_id, created_at DESC);
...
CREATE INDEX idx_album_media_album_pos ON album_media (album_id, position ASC);
...
CREATE TABLE IF NOT EXISTS album_comments (
    ...
    created_at DATETIME(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6),
    PRIMARY KEY (id),
    CONSTRAINT fk_album_comments_album FOREIGN KEY (album_id) REFERENCES albums(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;

CREATE INDEX idx_album_comments_album_created ON album_comments (album_id, created_at DESC);
CREATE INDEX idx_album_comments_media_id ON album_comments (media_id);   -- media_id NULL 行はインデックス対象外（MySQL/MariaDB デフォルト挙動）
...
| **hibiken/asynq**       | Beta: Redis ベースジョブキュー | v0.24+ |
| **oracle/oci-go-sdk**   | Prod: OCI Queue SDK          | v65+   |
...
var AlbumModule = fx.Module("album",
    fx.Provide(
        ...
        usecase.NewCreateHighlightVideoUsecase, // ユーザー指定のみ、自動生成しない
        ...
        handler.NewCreateHighlightVideoHandler, // ユーザー指定のみ
...
// Beta: self-hosted MySQL 8 (MariaDB 互換) / Prod: OCI MySQL HeatWave
...
// provideQueueAdapter: QueuePort 実装をプロバイダ設定から解決する
//   queue.provider = "redis-bullmq"  → RedisBullMQAdapter (Beta, asynq ベース)
//   queue.provider = "oci-queue"     → OCIQueueAdapter (Prod)
func provideQueueAdapter(cfg *config.Config, redis *redis.Client) application.QueuePort {
    switch cfg.QueueProvider {
    case "redis-bullmq":
        return adapter.NewRedisBullMQAdapter(asynq.NewClient(asynq.RedisClientOpt{Addr: cfg.RedisAddr}))
    case "oci-queue":
...
func startServer(
    ...
    createHighlightHandler *handler.CreateHighlightVideoHandler,
    queueWorker *worker.AlbumQueueWorker,
) {
    e := echo.New()
    ...
    // ハイライトビデオはユーザー指定の mediaIDs で生成（自動生成しない）
    e.POST("/api/albums/:album_id/highlight", createHighlightHandler.Handle)
    
    // QueuePort ワーカー起動 (Beta: Redis+BullMQ / Prod: OCI Queue)
...
| **E2E**      | 15%      | HTTP → DB → Cache → Queue 全フロー      | `testcontainers`, `docker-compose` |
...
// ハイライトビデオはユーザー指定のみ、自動生成しない。
// 旧 ShouldGenerateHighlight() は廃止された。代わりに、ユーザーが明示的に選んだ
// mediaIDs を渡す CreateHighlightVideo ユースケースをテストする。
func TestCreateHighlightVideo_RequiresUserSelectedMedia(t *testing.T) {
    uc := application.NewCreateHighlightVideoUsecase(...)
    _, err := uc.Execute(context.Background(), application.CreateHighlightVideoInput{
        AlbumID:  ulid.Make(),
        MediaIDs: []string{}, // 空
    })
    assert.ErrorIs(t, err, application.ErrNoMediaSelected)
}
...
- ハイライトビデオはユーザー指定のみ（QueuePort 経由で非同期結合、自動生成しない）
- 複数組織対応
...
| **フェーズ 4** | W7-8  | ユーザー指定ハイライト生成 (FFmpeg + QueuePort) | ジョブ失敗処理     |
...
| 項目                       | ステータス | 決定                         | 理由                            |
| -------------------------- | ---------- | ---------------------------- | ------------------------------- |
| **ハイライト生成エンジン** | DECIDED    | FFmpeg（HLS セグメント結合） | ユーザー指定素材のみ、ML/自動生成・AWS MediaConvert は不使用 |
| **キャッシュ戦略**         | DECIDED    | Redis (5min TTL)             | GetEventAlbum 頻出、低遅延必須  |
| **QueuePort 並列度**       | DECIDED    | 5 workers                    | ハイライトはユーザー指定のみ、自動トリガーなし |
...
- **asynq Task Queue (Beta)**: https://github.com/hibiken/asynq
- **OCI Queue (Prod)**: https://docs.oracle.com/iaas/Content/queue/home.htm
