...
│  ┌─────────────────────────────────────────────────────────┐│
│  │ HTTP Server | SQS Consumer | MySQL | Redis | S3    ││
│  └─────────────────────────────────────────────────────────┘│
└─────────────────────────────────────────────────────────────┘
                              △
                              │
...
5. Repository.Save() で MySQL に INSERT
...
| アダプタ                           | 技術          | 説明                                            |
| ---------------------------------- | ------------- | ----------------------------------------------- |
| **MySQLAuditRepository**           | pgx/v5        | MySQL 実装、月単位パーティション対応            |
| **RedisArchivalJobRepository**     | go-redis/v9   | アーカイバルジョブ状態キャッシング              |
| **S3ArchivalAdapter**              | aws-sdk-go-v2 | オブジェクトストレージ バケットのアーカイブ実装 |
| **MySQLRetentionPolicyRepository** | pgx/v5        | 保持ポリシー永続化                              |

### 5.4 外部サービスアダプタ

| アダプタ                     | 実装          | 説明                                                             |
| ---------------------------- | ------------- | ---------------------------------------------------------------- |
| **S3ArchivalServiceImpl**    | aws-sdk-go-v2 | オブジェクトストレージ バケットへの Parquet ファイルアップロード |
