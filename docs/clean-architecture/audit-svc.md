...
- **イベント配信**: QueuePort 非同期メッセージング（Beta: Redis+BullMQ/asynq、Prod: OCI Queue）
- **消費者**: 管理ダッシュボード、コンプライアンスレポート機能、ユーザ GDPR 開示
- **キー特性**: APPEND-ONLY パターン、PII 匿名化、オブジェクトストレージ（Garage / OCI Object Storage）による長期アーカイバル

### 1.3 アーキテクチャ原則
1. **ドメイン駆動設計**: ビジネスルール（不変性、監査証跡の完全性）をコア層に実装
2. **依存性逆転**: リポジトリ、キャッシュ、メッセージング参照はインターフェースで抽象化
3. **イベント駆動**: QueuePort 経由のメッセージによる疎結合なイベント処理（SQS は使用しない）
...
```
┌─────────────────────────────────────────────────────────────┐
│  External Systems (API, QueuePort, Object Storage, DB)       │
└─────────────────────────────────────────────────────────────┘
                              △
                              │
┌─────────────────────────────────────────────────────────────┐
...
│  ┌─────────────────────────────────────────────────────────┐│
│  │ HTTP Server | QueueConsumer | MySQL(MariaDB) | Redis | Garage/OCI OSS ││
│  └─────────────────────────────────────────────────────────┘│
└─────────────────────────────────────────────────────────────┘
                              △
                              │
...
│  ┌────────────┐ ┌────────────┐ ┌──────────────┐            │
│  │ HTTP       │ │ Queue      │ │ ObjStorage   │            │
│  │ Handlers   │ │ Consumers  │ │ Archival     │            │
│  └────────────┘ └────────────┘ └──────────────┘            │
│  ┌────────────┐ ┌────────────────────────────┐              │
...
| モデル                | 説明                                            | 主要属性                                                                                                                                         |
| --------------------- | ----------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| **AuditEntry**        | 不変監査ログエントリ                            | id (UUID char36), actor_id, actor_type, action, resource_type, resource_id, result, ip_address (VARCHAR(45)), user_agent, metadata (JSON), occurred_at (DATETIME(6)) |
| **RetentionPolicy**   | イベント種別の保持期間定義                      | id, event_type, retention_days, created_at                                                                                                       |
| **ArchivalJob**       | オブジェクトストレージ アーカイバル実行レコード | id, batch_id, status (PENDING/RUNNING/SUCCESS/FAILED), archived_at, rows_count, s3_path                                                          |
...
- **アーカイバル**: created_at が 2 年以上前のエントリはオブジェクトストレージ (Garage / OCI OSS) へアーカイブ必須
...
| **ArchivalJobFailed**          | オブジェクトストレージ アーカイバル失敗 | batch_id, error_reason, retry_count                        |
...
type ArchivalJob struct {
    ...
    RowsCount         int
    ArchivePath       string           // 互換: オブジェクトストレージ上の論理パス（bucket/key）
    ObjectStoragePath string           // Beta: Garage 上のフルパス / Prod: OCI Object Storage 上のフルパス
...
| ユースケース          | アクタ             | 説明                               | 主要入力                                          |
| --------------------- | ------------------ | ---------------------------------- | ------------------------------------------------- |
| **RecordAuditEntry**  | System (QueuePort) | Queue イベントから監査エントリを記録 | AuditEvent (Queue payload JSON)                 |
...
```go
// ArchivalService: オブジェクトストレージ アーカイバルポート
// 実装:
//   - Beta: GarageObjectStorageArchivalAdapter (S3 互換 API)
//   - Prod: OCIObjectStorageArchivalAdapter
type ArchivalService interface {
    // ArchiveEntries: エントリリストをオブジェクトストレージへアーカイブ
    // archivePath はバケット内の論理パス（例: audits/2026/01/batch-xxxx.parquet）
    ArchiveEntries(ctx context.Context, entries []*domain.AuditEntry, archivePath string) (objectStoragePath string, err error)
...
| -------------------------- | ------------- | ------------------ | ------------------------------------------ |
| **QueueEventHandler**      | -             | (async)            | QueuePort メッセージ受信 → RecordAuditEntry 実行 |
...
| アダプタ                           | 技術          | 説明                                            |
| ---------------------------------- | ------------- | ----------------------------------------------- |
| **MySQLAuditRepository**           | go-sql-driver/mysql | MySQL/MariaDB 実装、月単位パーティション対応（MariaDB RANGE パーティション） |
| **RedisArchivalJobRepository**     | go-redis/v9         | アーカイバルジョブ状態キャッシング                                           |
| **ObjectStorageArchivalAdapter**   | Beta: AWS SDK v2 (Garage互換) / Prod: oci-go-sdk | オブジェクトストレージ バケットへのアーカイブ実装 |
| **MySQLRetentionPolicyRepository** | go-sql-driver/mysql | 保持ポリシー永続化                                                           |

### 5.4 外部サービスアダプタ

| アダプタ                     | 実装          | 説明                                                             |
| ---------------------------- | ------------- | ---------------------------------------------------------------- |
| **ObjectStorageArchivalServiceImpl** | Garage/OCI OSS SDK | オブジェクトストレージ バケットへの Parquet ファイルアップロード |
| **RedisCache**                       | go-redis/v9        | Redis キャッシュ実装                                             |
| **QueueEventPublisher**              | QueuePort          | QueuePort (Beta: Redis/asynq, Prod: OCI Queue) へのイベント発行  |
| **PostfixSMTPNotificationAdapter**   | net/smtp           | GDPR 完了メール送信 (Postfix+Dovecot+Rspamd / CoreServerV2)       |
...
// MapQueueEventToInput: QueuePort メッセージ (JSON) → RecordAuditEntryInput
func (m *AuditMapper) MapQueueEventToInput(event *queue.Message) (*application.RecordAuditEntryInput, error) {
...
> **互換性方針**: Beta は MySQL 8.x、Prod は OCI MySQL HeatWave。いずれも **MariaDB 10.6+ 互換の SQL に限定**。
> PostgreSQL 固有記法（`UUID` 型、`INET`、`JSONB`、`TIMESTAMPTZ`、`PARTITION OF`、`USING GIN`、`gen_random_uuid()`）は使用せず、
> `CHAR(36)`（UUID 文字列）、`VARCHAR(45)`（IPv6 最大長）、`JSON`、`DATETIME(6)`、`PARTITION BY RANGE (YEAR(...)*100 + MONTH(...))`、
> JSON カラム用の仮想カラム + インデックス、`UUID()` で代替する。

```sql
-- MySQL 8.x / MariaDB 10.6+ スキーマ（月単位 RANGE パーティション）
...
CREATE TABLE IF NOT EXISTS audit_entries (
    ...
    ip_address VARCHAR(45) NOT NULL,  -- IPv6 最大長
    user_agent TEXT,
    metadata JSON,
    occurred_at DATETIME(6) NOT NULL,
    created_at DATETIME(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6),
    -- CHECK 制約は MySQL 8 / MariaDB 10.2+ で有効
    CHECK (actor_type IN ('USER', 'SERVICE', 'SYSTEM')),
    CHECK (action IN ('READ', 'WRITE', 'DELETE', 'LOGIN', 'LOGOUT', 'PERMISSION_CHECK', 'ADMIN_ACTION')),
    CHECK (resource_type IN ('USER', 'ORG', 'MEDIA', 'ALBUM', 'EVENT', 'MESSAGE')),
    CHECK (result IN ('ALLOWED', 'DENIED', 'ERROR')),
    PRIMARY KEY (id, created_at)  -- パーティションキーを含める必要あり
)
...
-- JSON 検索は仮想カラム + 通常インデックスで代替（USING GIN は非サポート）
ALTER TABLE audit_entries
    ADD COLUMN metadata_event_type VARCHAR(100)
        GENERATED ALWAYS AS (JSON_UNQUOTE(JSON_EXTRACT(metadata, '$.event_type'))) VIRTUAL,
    ADD INDEX idx_audit_meta_event_type (metadata_event_type);
...
-- UUID() は MySQL/MariaDB 共通関数
INSERT INTO retention_policies (id, event_type, retention_days) VALUES
    (UUID(), 'LOGIN', 365),
    (UUID(), 'DATA_CHANGE', 2555),  -- 7 years
    (UUID(), 'ADMIN_ACTION', 2555),
    (UUID(), 'PERMISSION_CHECK', 90);
...
CREATE TABLE IF NOT EXISTS archival_jobs (
    ...
    archive_path TEXT,          -- 論理パス（bucket/key）
    object_storage_path TEXT,   -- Beta: Garage 上のフル URI / Prod: OCI Object Storage 上のフル URI
...
| ライブラリ                  | 用途                | バージョン |
| --------------------------- | ------------------- | ---------- |
| **go-sql-driver/mysql**     | MySQL/MariaDB 互換ドライバ | v1.8+   |
...
var AuditModule = fx.Module("audit",
    fx.Provide(
        ...
        provideRedis,
        provideObjectStorageArchiver,     // Beta: Garage / Prod: OCI OSS
        provideQueueAdapter,              // Beta: Redis+BullMQ / Prod: OCI Queue
        
        // リポジトリ実装
        repository.NewMySQLAuditRepository,
        repository.NewMySQLRetentionPolicyRepository,
        repository.NewMySQLArchivalJobRepository,
        ...
        // ハンドラ
        handler.NewQueueEventHandler,     // QueuePort 経由でイベント消費
...
// provideMySQLDB: MySQL/MariaDB コネクションプール
func provideMySQLDB(cfg *config.Config) (*sql.DB, error) {
...
// provideRedis: Redis クライアント（Beta: self-hosted / Prod: OCI Cache with Redis）
...
// provideObjectStorageArchiver: ArchivalService のアダプタ選択
//   storage.provider = "garage"   → GarageObjectStorageArchivalAdapter (S3互換APIクライアント)
//   storage.provider = "oci-oss"  → OCIObjectStorageArchivalAdapter
func provideObjectStorageArchiver(cfg *config.Config) application.ArchivalService {
    switch cfg.StorageProvider {
    case "garage":
        return adapter.NewGarageObjectStorageArchivalAdapter(cfg.StorageEndpoint, cfg.StorageBucket, cfg.StorageAccessKey, cfg.StorageSecretKey)
    case "oci-oss":
        return adapter.NewOCIObjectStorageArchivalAdapter(cfg.OCINamespace, cfg.OCIBucket, cfg.OCIRegion)
    default:
        panic("unsupported storage.provider: " + cfg.StorageProvider)
    }
}

// provideQueueAdapter: QueuePort アダプタ選択
//   queue.provider = "redis-bullmq" → Beta
//   queue.provider = "oci-queue"    → Prod
func provideQueueAdapter(cfg *config.Config, redis *redis.Client) application.QueuePort {
    ...
    default:
        panic("unsupported queue.provider: " + cfg.QueueProvider)
    }
}
...
func startServer(
    ...
    queueHandler *handler.QueueEventHandler,
) {
    e := echo.New()
    e.GET("/api/audits", handler.Handle)
    
    go queueHandler.StartConsumer()
...
│   │   ├── MySQL_retention_policy_repository.go
│   │   └── MySQL_archival_job_repository.go
│   └── external/
...
│   ├── adapter/
│   │   └── MySQL_audit_repository_test.go
│   ├── integration/
│   │   └── sqs_to_MySQL_test.go
│   └── fixtures/
│       ├── audit_entries.json
│       └── sample_events.json
│
...
| **E2E**      | 25%      | QueuePort → DB → キャッシュ全フロー  | `docker-compose`, `testcontainers` |
...
// adapter/MySQL_audit_repository_test.go: 統合テスト
...
func TestMySQLAuditRepository_Save(t *testing.T) {
    if testing.Short() {
    ...
    pool := MySQL.Pool()
    repo := repository.NewMySQLAuditRepository(pool)
...
- **ArchivalError**: オブジェクトストレージ (Garage / OCI OSS) アーカイバル失敗
...
- **トレース ID**: QueuePort メッセージ ID を全ログに包含
...
### 12.2 目標状態
- MySQL APPEND-ONLY ログ
- 複数マイクロサービス間の一元監査
- GDPR 削除リクエスト自動化
- オブジェクトストレージ (Garage / OCI OSS) 冷保存アーカイバル

### 12.3 マイグレーション手順

| フェーズ       | 期間  | 作業                                                 | リスク             |
| -------------- | ----- | ---------------------------------------------------- | ------------------ |
| **フェーズ 1** | W1-2  | MySQL テーブル作成、パーティション設定               | テーブル設計誤り   |
| **フェーズ 2** | W3-4  | QueuePort Consumer 実装、RecordAuditEntry テスト     | メッセージ損失     |
| **フェーズ 3** | W5-6  | QueryAuditLogs API 実装、キャッシング                | キャッシュ一貫性   |
| **フェーズ 4** | W7-8  | オブジェクトストレージ アーカイバル、GDPR 匿名化実装 | データ消失         |
...
| 項目                       | ステータス | 決定               | 理由                             |
| -------------------------- | ---------- | ------------------ | -------------------------------- |
| **メッセージフォーマット** | DECIDED    | JSON (QueuePort 共通) | 汎用性、デシリアライズ容易       |
...
| **Queue リトライ**         | DECIDED    | DLQ + 3 回リトライ | 一時的なエラーに対応             |
...
- **MySQL パーティショニング**: https://www.MySQL.org/docs/current/ddl-partitioning.html
- **Garage Object Storage**: https://garagehq.deuxfleurs.fr/
- **OCI Object Storage Lifecycle**: https://docs.oracle.com/iaas/Content/Object/Tasks/usinglifecyclepolicies.htm
