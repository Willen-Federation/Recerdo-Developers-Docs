...
│  Gin, PostgreSQL, Redis, AWS Cognito, SQS          │
└─────────────────────────────────────────────────────┘
                         ▲
                         │ (依存)
┌─────────────────────────────────────────────────────┐
...
|---|---|---|---|
| LoginHandler | POST | /api/auth/login | メール・パスワードでログイン |
...
| リポジトリ実装 | 技術スタック | 説明 |
|---|---|---|
| PostgresUserRepository | Database/sql + sqlc | ユーザーレコード CRUD |
| PostgresUserSessionRepository | Database/sql + sqlc | セッションレコード CRUD |
| PostgresDeviceRegistrationRepository | Database/sql + sqlc | デバイスレコード CRUD |
| RedisBlockedTokenRepository | Redis (go-redis/v9) | トークンブロックリスト |
| PostgresRefreshTokenGrantRepository | Database/sql + sqlc | リフレッシュトークン払い出し記録 |
...
func provideDatabaseConnection() (*sql.DB, error) {
	return sql.Open("postgres", "postgres://user:pass@db:5432/recuerdo")
}
...
└── k8s/
    ...
    └── statefulset-postgres.yaml # PostgreSQL StatefulSet (optional)
```
...
// Integration Test: PostgreSQL UserRepository
...
- **DatabaseError**: PostgreSQL クエリ失敗
...
### 11.4 キャッシング
- **User**: PostgreSQL primary; Redis L1 キャッシュなし (一貫性重視)
