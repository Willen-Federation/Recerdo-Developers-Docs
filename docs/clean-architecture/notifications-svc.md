...
func NewPostfixSMTPAdapter(cfg SMTPConfig) (*PostfixSMTPAdapter, error) {
	// 設定整合性の早期検証: 片方だけ設定された誤設定を起動時に検出する
	switch {
	case cfg.Username == "" && cfg.Password == "":
		// AUTH 無しモード（Postfix 側が submission relay 制限で運用）
	case cfg.Username != "" && cfg.Password != "":
		// 通常の AUTH 有りモード
	default:
...
// sanitizeHeaderValue はヘッダ値から CR / LF を取り除き、ヘッダインジェクション
// （BCC 追加・追加ヘッダ注入）を防ぐ。値に混入していれば設定エラーとして拒否する。
func sanitizeHeaderValue(name, value string) (string, error) {
	if strings.ContainsAny(value, "\r\n") {
		return "", fmt.Errorf("header %q contains CR/LF", name)
	}
	return value, nil
}
...
func (a *PostfixSMTPAdapter) SendEmail(ctx context.Context, notif *domain.Notification, toAddress string) error {
	...
	// 宛先・件名は必ず検証・エンコードしてからヘッダに組み込む
	to, err := mail.ParseAddress(toAddress)
	if err != nil {
		return fmt.Errorf("invalid recipient address: %w", err)
	}
	fromHeader, err := sanitizeHeaderValue("From", a.from)
	if err != nil {
		return err
	}
	toHeader, err := sanitizeHeaderValue("To", to.Address)
	if err != nil {
		return err
	}
	// 非 ASCII 件名は RFC 2047（MIME encoded-word）でエンコード
	subject := mime.QEncoding.Encode("utf-8", notif.Title)
	if _, err := sanitizeHeaderValue("Subject", subject); err != nil {
		return err
	}
	...
	// STARTTLS は必須。AUTH より前に必ず昇格させ、平文での資格情報送信を防ぐ。
	...
	// AUTH は username/password が両方設定されている場合のみ実行する。
	// 片方だけの誤設定は NewPostfixSMTPAdapter で既に拒否されている。
	if a.username != "" && a.password != "" {
		if ok, _ := c.Extension("AUTH"); !ok {
			return fmt.Errorf("smtp server %s does not advertise AUTH after STARTTLS", addr)
		}
		if err := c.Auth(smtp.PlainAuth("", a.username, a.password, a.host)); err != nil {
			return err
		}
	}
...
### 5.3 PostfixSMTP 利用条件 { #postfixsmtp-利用条件 }
...
| 2026-04-19 | 横断レビュー（`464267` マージ後）  | PostfixSMTP 利用条件が MS / CA で表現が異なる              | §5.3 に明示 ID `{ #postfixsmtp-利用条件 }` を付与、MS 側は `{ #メール通知条件 }` で揃え、両表を 5 条件で同期し policy.md §8.1 へもリンク |
