...
### 1.1 目的
Notification Service は Recuerdo プラットフォーム全体のプッシュ通知・アプリ内通知・メール通知を一元管理する。Messaging Service、Events Service、Album Service からのイベントを SQS 経由で受け取り、**FCM（Firebase Cloud Messaging）を主軸としたプッシュ通知**を配信する。オンプレミスサーバーによるメール通知はセキュリティ要件・FCMトークン未登録ユーザー・法的通知など特定条件下のみで利用する（条件詳細は「5.3 SES利用条件」を参照）。ユーザー設定に基づいた通知ルーティング、デバイストークン管理、配信状態トラッキング、重複排除を提供し、低コストかつスケーラブルな通知基盤を実現する。
...
- **コスト効率**: 無料/低コス トサービス（FCM+メール）の活用
...
│  Gin, MySQL, Redis, AWS FCM, メール, SQS             │
└──────────────────────────────────────────────────────────┘
                          ▲
                          │ (依存)
┌──────────────────────────────────────────────────────────┐
...
### 2.2 依存性ルール
外側のレイヤーは内側に依存し、内側は外側に依存しない。データフロー: 入力アダプタ（REST/SQS）→ ユースケース → 出力アダプタ（DB/FCM/メール）。外側のレイヤー間通信はポート（インターフェース）経由のみ。ドメイン層は業務ロジック、フレームワーク非依存。
...
#### EmailAdapter
##### TODO
 - [ ] メールについてはオンプレミスサーバーでの利用とするので、ハードコードしない。
 - [ ] 通信はSMTPプロトコルを利用する。または、メール送信のみCloudFlareなどの無料サービスを利用することも検討する。
 - [ ] 以降についてはSESにおける文言は考慮しない。また、クラウド利用したメール送信をする場合についても、Feature Flag等を利用して、今後のフェーズに応じて検討する。
