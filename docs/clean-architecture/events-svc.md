...
- **イベント駆動型アーキテクチャ**：EventCreated、EventArchived などをSQS経由で発行し、Album Service、Timeline Service などと疎結合
...
┌─────────────────────────────────────────────────────┐
│  フレームワーク＆ドライバ層                          │
│  (Web: Gin, DB: PostgreSQL, Queue: SQS, Redis)     │
└────────────┬──────────────────────────────────────┘
             │
┌────────────▼──────────────────────────────────────┐
│  インターフェースアダプタ層                        │
...
7. EventCreatedEvent を SQS 発行（Event Emitter）
8. EventID を含む Response を返却

**Post-conditions**:
- Event が DB に DRAFT 状態で保存
- EventCreated が SQS に発行 → Album Service, Timeline Service がリッスン
...
|-----------|------|------|----------------|
| **PostgresEventRepository** | Event | `database/sql` + sqlc | リスト結果 → Redis (TTL 5min) |
| **PostgresEventInvitationRepository** | EventInvitation | `database/sql` + sqlc | 保留中招待 → Redis (TTL 10min) |
| **PostgresEventParticipantRepository** | EventParticipant | `database/sql` + sqlc | Count → Redis (TTL 2min) |

### 5.4 外部サービスアダプタ

| アダプタ | 外部サービス | 実装 | エラーハンドリング |
|---------|----------|------|----------------|
| **SQSEventEmitter** | AWS SQS | `github.com/aws/aws-sdk-go-v2` | リトライ 3回、デッドレターキュー |
| **SNSNotificationAdapter** | AWS SNS | `aws/aws-sdk-go-v2` | メール送信失敗→ログ＋監視アラート |
| **AuthServiceClient** | Auth Service | gRPC | タイムアウト 5sec、サーキットブレーカー |
...
### 6.3 メッセージブローカー
- **SQS キューURL**: `https://sqs.{region}.amazonaws.com/{account}/recuerdo-events`
- **メッセージ仕様**: 
  - Type: JSON
  - TTL: 4日（SQS デフォルト）
  - Dead Letter Queue: `recuerdo-events-dlq`（max receive count: 3）
- **消費者**: Album Service, Timeline Service, Notification Service
...
| `github.com/aws/aws-sdk-go-v2/sqs` | SQS クライアント | v1.26 |
    ...
    return fx.Module("events-service",
        ...
        fx.Provide(
            func(db *sql.DB) adapter.EventRepository {
                return adapter.NewPostgresEventRepository(db)
            },
            func(db *sql.DB) adapter.EventInvitationRepository {
                return adapter.NewPostgresEventInvitationRepository(db)
            },
            func(db *sql.DB) adapter.EventParticipantRepository {
                return adapter.NewPostgresEventParticipantRepository(db)
            },
        ),
        // 外部サービスアダプタ
        fx.Provide(
            func(sqsClient *sqs.Client) application.EventEmitter {
                return adapter.NewSQSEventEmitter(sqsClient)
            },
        ),
        // ユースケース（アプリケーション層）
        fx.Provide(
            func(
                ...
                return application.NewCreateEventUseCase(eventRepo, emitter, permSvc)
            },
            // その他のユースケース...
        ),
        // ハンドラ登録
...
func provideSQSClient(cfg *config.AWSConfig) *sqs.Client {
    return sqs.NewFromConfig(cfg.AWSSDKConfig)
}
...
│   │   │   ├── postgres_invitation_repo.go
│   │   │   └── postgres_participant_repo.go
│   │   ├── external/
...
- **イベント駆動の境界**：ドメインイベント → Event Emitter (SQS) → 他サービス（疎結合）
...
| **統合テスト** | 20% | Handler + UseCase + Repo（実 DB） | `testcontainers-go` (PostgreSQL) |
...
func TestCreateEventUseCase_Integration(t *testing.T) {
    // Arrange: testcontainers で PostgreSQL スピンアップ
    ...
    eventRepo := adapter.NewPostgresEventRepository(db)
...
| フェーズ | 実施内容 | 期間 | 依存関係 |
|---------|--------|------|--------|
| **1. インフラ準備** | PostgreSQL テーブル作成、SQS キュー作成、Redis namespace 設定 | 1週間 | なし |
