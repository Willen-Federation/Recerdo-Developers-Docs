...
┌─────────────────────────────────────────────────────┐
│  フレームワーク＆ドライバ層                          │
│  (Web: Gin, DB: MySQL, Queue: SQS, Redis)     │
└────────────┬──────────────────────────────────────┘
             │
┌────────────▼──────────────────────────────────────┐
│  インターフェースアダプタ層                        │
...
#### TODO
- [ ] SQS、SNSについては、Oracleを利用するか、OSSを利用してSelf-Hostedとするので、本内容については要検討
...
| フェーズ                      | 実施内容                                                 | 期間  | 依存関係  |
| ----------------------------- | -------------------------------------------------------- | ----- | --------- |
| **1. インフラ準備**           | MySQL テーブル作成、SQS キュー作成、Redis namespace 設定 | 1週間 | なし      |
