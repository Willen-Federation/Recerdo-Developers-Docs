...
│  Gin + Traefik (Beta) / OCI API Gateway (Prod),     │
│  Redis, QueuePort (BullMQ / OCI Queue), OTel, K8s   │
└─────────────────────────────────────────────────────┘
                         ▲
                         │ (依存)
┌─────────────────────────────────────────────────────┐
...
- 必要なイベントが QueuePort (Beta: Redis+BullMQ / Prod: OCI Queue) に発行される
...
| QueueEventPublisher           | QueuePort (Beta: Redis+BullMQ / Prod: OCI Queue) | イベントをキューに発行 |
...
### 6.3 メッセージブローカー
- **Broker**: QueuePort 抽象
  - **Beta**: Redis + BullMQ / asynq (`RedisBullMQAdapter`)
  - **Prod**: OCI Queue (`OCIQueueAdapter`)
  - AWS SQS / SNS は使用しない
- **Topics** (論理名):
...
var Module = fx.Module("infrastructure",
	fx.Provide(
		...
		// AWS SDK (Cognito only — AWS で唯一許可されたサービス)
		provideAWSConfig,
		provideCognitoClient,

		// QueuePort adapter (Beta: Redis+BullMQ, Prod: OCI Queue)
		provideQueueAdapter,
...
| 6   | イベント発行             | QueuePort (Beta: Redis+BullMQ / Prod: OCI Queue) | Kafka, AWS SQS                 | AWS 依存排除方針 (Cognito を除く) |
...
- Traefik Proxy (Beta): https://doc.traefik.io/traefik/
- AWS Cognito Documentation (認証のみ): https://docs.aws.amazon.com/cognito/
