...
- **ハイライトビデオ生成**: アルバム内に10以上のメディアが存在する場合の非同期ハイライトビデオ自動生成
...
4. **マーケティング**: イベントのハイライトビデオ（抜き出しビデオ）を自動生成し、SNS共有用に利用したい
...
| **HighlightVideo** | アルバムから自動生成されたハイライトビデオ。非同期ジョブ (asynq) により生成 | id (ULID), album_id, video_media_id (Storage Service参照), duration_secs, status (PENDING/READY/FAILED), generated_at, metadata_json |
...
- **Highlight Video Trigger**: アルバムに10個以上のメディアが存在する場合のみハイライトビデオ生成ジョブが自動トリガーされる
...
|---|---|---|
| **AlbumRepository** | PostgresAlbumRepository | PostgreSQL: albums テーブル |
| **AlbumMediaRepository** | PostgresAlbumMediaRepository | PostgreSQL: album_media テーブル |
| **AlbumCommentRepository** | PostgresAlbumCommentRepository | PostgreSQL: album_comments テーブル |
| **HighlightVideoRepository** | PostgresHighlightVideoRepository | PostgreSQL: highlight_videos テーブル |
...
| **EventPublisher** | SQSEventPublisher | AWS SQS (album-events queue) |
| **HighlightVideoJobQueue** | AsynqJobQueue | Asynq (Redis queue, worker pool) |
...
| 項目 | 選択 | 説明 |
|---|---|---|
| **RDBMS** | PostgreSQL 14+ | メインデータストア |
| **Driver** | pgx (pq) | Go PostgreSQL driver |
| **Connection Pool** | pgx.ConnPool | 接続プーリング（デフォルト設定） |
| **Migration** | sql-migrate / flyway | DB スキーマ管理 |
| **Cache** | Redis 6.0+ | album_media リスト キャッシュ (TTL: 5分) |
| **Redis Driver** | go-redis/redis | Redis クライアント |
| **Queue** | Asynq (Redis) | ハイライトビデオ生成ジョブキュー |
...
| **hibiken/asynq** | 非同期ジョブキュー | Infrastructure |
| **aws/aws-sdk-go** | SQS 通信 | Infrastructure |
| **lib/pq** / **jackc/pgx** | PostgreSQL ドライバ | Infrastructure |
...
func main() {
    app := fx.New(
        ...
        fx.Provide(infra.NewPostgresDB),
        fx.Provide(infra.NewRedisClient),
        fx.Provide(infra.NewAsynqClient),
        fx.Provide(infra.NewSQSPublisher),
        
        // Repository Providers
        fx.Provide(adapter.NewPostgresAlbumRepository),
        fx.Provide(adapter.NewPostgresAlbumMediaRepository),
        fx.Provide(adapter.NewPostgresCommentRepository),
        fx.Provide(adapter.NewPostgresHighlightVideoRepository),
        ...
        fx.Provide(adapter.NewSQSEventPublisher),
        fx.Provide(adapter.NewAsynqJobQueue),
...
│   └── infra/
│       ├── config.go                    # 設定読み込み
│       ├── postgres.go                  # PostgreSQL 接続初期化
│       ├── redis.go                     # Redis 接続初期化
│       ├── asynq.go                     # Asynq クライアント初期化
│       ├── sqs.go                       # SQS クライアント初期化
│       ├── logger.go                    # ロガー初期化
...
| **Adapter (Repository)** | Integration Test | テスト用 PostgreSQL (testcontainers) | 90%+ |
| **External Services** | Contract Test | HTTP モックサーバー (httptest) | - |
| **E2E** | Integration Flow | 実 PostgreSQL + Redis | 主要ユースケース |
...
```go
// test/integration/repository/postgres_album_test.go
...
func TestPostgresAlbumRepository_Save(t *testing.T) {
...
| # | 質問 | ステータス | 決定 |
|---|---|---|---|
| 1 | **Media storage format**: 写真と動画で異なる処理が必要か？ | ⏳ Pending | 今後 Storage Service の仕様に基づいて決定。現在は metadata_id で統一 |
| 2 | **Highlight video algorithm**: どのメディアを選出するのか？ | ⏳ Pending | AI ベースの選出 or ユーザー指定 → Product チームに確認予定 |
| 3 | **Album soft delete retenetion**: 削除済みアルバムの保持期間は？ | ⏳ Pending | 推奨: 90日間。その後物理削除 |
| 4 | **Comment moderation**: コメント内容の審査機能は必要か？ | ⏳ Pending | Content Moderation Service との連携検討 |
| 5 | **Real-time notifications**: アルバム更新時の Push 通知は必要か？ | ⏳ Pending | Notification Service との連携検討 |
| 6 | **Bulk operations**: 複数メディアの一括削除は実装対象か？ | ⏳ Pending | MVP では個別削除のみ。v2 で実装検討 |
| 7 | **Cache invalidation strategy**: Redis キャッシュの無効化戦略は？ | ✅ Decided | TTL 5分 + イベント発行時の即座削除パターン採用 |
| 8 | **Concurrent media reorder**: 同時に複数ユーザーが reorder した場合の競合制御は？ | ⏳ Pending | Optimistic locking (version field) または Pessimistic locking の検討 |
| 9 | **Highlight video SLA**: 生成ジョブの完了時間は？ | ⏳ Pending | 推奨: 5分以内（優先度: normal） |
| 10 | **Cross-org album visibility**: 異なる org のアルバム参照は可能か？ | ✅ Decided | org_id でスコープ制御。クロスオーグ参照は不可 |
