...
既存のマイクロサービス index では「**メッセージング: Amazon SQS / SNS**」と記載されているが、Recerdo は以下のフェーズで **異なる基盤** を使う必要がある：

- **Closed Beta**: VPS + レンタルサーバー構成 → SQS 利用不可（AWS 依存を最小化）
- **本番（OCI ファースト）**: OCI Queue Service または AWS SQS への切替
- **将来**: ユーザー地域拡張に応じてリージョン別に使い分け（例：EU は GCP Pub/Sub）
...
|---|---|---|---|
| **Redis + BullMQ / Sidekiq** ⭐ | セルフホスト | 軽量・運用容易・Node/Rails 親和性高 | **Beta（第一推奨）** |
| Redis + Asynq | セルフホスト（Go） | BullMQ の Go 版相当 | Beta（Go 中心なら） |
| RabbitMQ | セルフホスト | AMQP 完備・複雑ルーティング可能 | 中規模本番 |
| NATS JetStream | セルフホスト | 軽量・高スループット・ストリーミング向け | 将来のイベント駆動拡張時 |
| Apache Kafka / Redpanda | セルフホスト | 大規模イベント・ログ処理 | 大規模本番（過剰気味） |
| **OCI Queue Service** | マネージド | OCI ネイティブ・AMQP 互換 | **本番（第一候補）** |
| **AWS SQS** | マネージド | 堅牢・運用ゼロ | 本番（Cognito と併用） |
| GCP Pub/Sub | マネージド | 対象外（クラウド選定より） | — |

### 2.2 詳細比較

| 観点 | BullMQ (Redis) | Sidekiq (Redis) | RabbitMQ | NATS JetStream | OCI Queue | AWS SQS |
|---|---|---|---|---|---|---|
| ランタイム | Node.js | Ruby | Erlang | Go | マネージド | マネージド |
| プロトコル | Redis | Redis | AMQP 0-9-1 | NATS protocol | AMQP 1.0 | 独自 HTTPS |
| 保証 | At-least-once | At-least-once | At-least-once / exactly-once | At-least-once | At-least-once | At-least-once |
| DLQ | ✅ 内蔵 | ✅ 内蔵 | ✅ | ✅ | ✅ | ✅ |
| 遅延実行 | ✅ 秒精度 | ✅ 分精度 | ✅ プラグイン | ✅ | ✅ | ✅ (最大 15分) |
| レート制限 | ✅ | ❌ 自前 | ❌ | ❌ | ❌ | ❌ |
| 優先度 | ✅ | ✅ Pro のみ | ✅ | ❌ | ❌ | ❌ |
| 最小運用コスト | $5/月 VPS | $5/月 VPS | $10/月 VPS | $5/月 VPS | $0〜 | $0〜 (従量) |
| 学習曲線 | 低 | 低 | 中 | 中 | 低 | 低 |
...
フォールバック先として **AWS SQS** も同じ Port 実装で選択可能。
...
flowchart LR
  ...
  subgraph Adapters["Queue Adapters (差替え可能)"]
    ...
    Sidekiq["Sidekiq Adapter"]
    OCIQ["OCI Queue Adapter"]
    SQS["SQS Adapter"]
...
| `notification.email` | SES メール配信 | 500 msg/日 | 1日 |
...
### 4.4 AWS SQS Adapter（本番代替・MENA 地域等）

- FIFO キュー: `audit.events.fifo`（監査系のみ厳密順序）
- Standard キュー: その他（ベストエフォート順序）
- DLQ: 別 Queue を `redrivePolicy` で紐付け
- 長いポーリング: 20秒、コスト最適化
...
- **データ差分監視**: CloudWatch / OCI Monitoring で処理レート・失敗率を 1 分粒度で比較
...
- [AWS SQS — Amazon Simple Queue Service](https://aws.amazon.com/sqs/)
