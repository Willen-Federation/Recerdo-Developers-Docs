...
- 禁止キーワード検証ルール（`core/policy.md`）に沿って、採用文脈での混入がないことを確認。
...
- **`microservice/notifications-svc.md`**: §9「横断標準の適用」、§10「レビュー指摘の反映記録」を新設。
- **`clean-architecture/notifications-svc.md`**: §14「変更履歴・レビュー記録（追加設計プラン反映）」を新設。
