...
- ❌ **クラウド事業者名をドメイン層にハードコードしない**（"oci" / "aws" / "garage" を書かない）
...
!!! warning "禁止パターン"
    ...
    ```
    ```go
    // ✅ GOOD: 具体的な設定値で判断（Port/Adapter 経由）
    queue := NewQueue(cfg.QueueProvider) // "redis-bullmq" | "oci-queue"
    ```

### 2.3 Feature Flag の評価ポリシー

- 全サービスは [Feature Flag Svc（recerdo-feature-flag）](../microservice/feature-flag-system.md) に `EvaluateFlag` で問い合わせる
...
- `UPPER_SNAKE_CASE`
- ドメイン別プレフィックス：`QUEUE_*`、`STORAGE_*`、`MAIL_*`、`AUTH_*`、`DB_*`、`MEDIA_*`
...
!!! warning "削除済みの環境変数オプション"
    以下のオプションはポリシーにより **削除** されました。コードや設定にこれらが残っている場合は除去してください：
    - `STORAGE_PROVIDER=aws-s3` / `STORAGE_PROVIDER=minio`
    - `QUEUE_PROVIDER=aws-sqs`
    - `MAIL_PROVIDER=aws-ses` / `MAIL_PROVIDER=sns`
    - AWS SNS / DynamoDB / RDS / Aurora / ElastiCache / CloudFront / Lambda / CloudWatch / ECS / Fargate / EC2 関連の全ての `*_PROVIDER` 値
...
- **Beta**: XServer VPS の `.env.local`（ファイルパーミッション 600）+ `sops + age` 暗号化で Git 管理
- **本番**: **OCI Vault** から起動時に環境変数へ注入
