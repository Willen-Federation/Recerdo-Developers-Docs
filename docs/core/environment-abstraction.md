...
- メッセージキュー実装の切替（Redis+BullMQ → OCI Queue / AWS SQS）
- オブジェクトストレージの切替（MinIO → OCI Object Storage / AWS S3）
- 認証プロバイダ追加（Cognito 単独 → Cognito + Auth0 などの併用）
- 特定機能の段階的ロールアウト（%ベース／組織ベース／IP ベース）
- 障害時の即時機能停止（Kill Switch）

**根拠**: Notion レビューコメント（2026-04-18）「ハードコードするとシステムの改修が大変となります。最終的には、システムの切り替えが容易になるように、FeatureFlagやソフト変更で対応できるシステムにしてください。」
...
| `DATABASE_URL` | `postgres://...@vps-db:5432/recerdo` | `postgres://...@autonomousdb.ap-tokyo-1.oci.oraclecloud.com:5432/...` | DB 接続文字列 |
