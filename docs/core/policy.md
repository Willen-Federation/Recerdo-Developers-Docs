...
## 8. 追加設計プラン（大規模類似サービス参照） { #8-大規模類似サービス参照反復版 }

本節は、既存方針に対して「設計 → 分析 → 課題抽出 → レビュー反映」を反復するための補助基準とする。

| 設計テーマ | 参照した大規模サービスの一般的モデル | Recuerdo での反映内容 |
| --- | --- | --- |
| 通知経路最適化 | Push-first + 条件付き Email フォールバック | FCM を既定、Postfix はセキュリティ/法的通知など限定条件のみ |
| 非同期処理信頼性 | at-least-once 配信 + DLQ + 冪等ハンドリング | QueuePort 標準化、再試行 3 回、DLQ 監査を全サービス共通化 |
| 運用監査 | 監査イベント一元化 + 長期保管 | audit-svc 集約、Warm/Cold アーカイブ方針を維持 |
| 段階移行 | 環境差分をアダプタ切替で吸収 | Beta/Prod を Port & Adapter + Feature Flag で統一 |
...
4. `microservice/index.md` と `clean-architecture/index.md` の横断表を更新し、差分を可視化する。
...
最終更新: 2026-04-19 ポリシー適用（§8 追加設計プラン反復版・Iteration-02 整理）
